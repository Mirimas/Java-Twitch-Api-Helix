...
import ru.mirrobot.twitch.api.auth.grants.implicit.AuthenticationCallbackServer;
import ru.mirrobot.twitch.api.auth.grants.implicit.AuthenticationException;
...
public class Authenticator {
    ...
    public boolean awaitAccessToken(URL authUrl, URL successUrl, URL failUrl) {
        ...
        try {
            ...
            authenticationException = new AuthenticationException(e.toString(), null, e);
            return false;
        }

        if (server.hasAuthenticationError() || server.getAccessToken() == null) {
            authenticationException = server.getAuthenticationException();
            return false;
        }
